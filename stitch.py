...
var PERSONA_META = {
  ...
  jamie: {
    fullName: 'Jamie the Pathfinder', shortName: 'Jamie', color: '#5C8841',
...
// ── Map screen ────────────────────────────────────────────────────────────
var PERSONA_CHIPS = { jordan:'Rising Star', alex:'Level Upper', taylor:'City Hopper', jamie:'Pathfinder' };
...
function syncMapScreen() {
  ...
  function _swapMapName(html) {
    // Single-pass replacement avoids cascading substitutions when the target
    // name itself contains one of the source names (e.g. "Taylor & Riley").
    var pat = _MAP_NAMES.join('|');
    html = html.replace(new RegExp('\\b(?:' + pat + ")'s\\b", 'g'), poss);
    html = html.replace(new RegExp('\\b(?:' + pat + ')\\b', 'g'), meta.shortName);
    return html;
  }
...
function refreshNotesCard(chNum) {
  ...
  // Build "Copy all" text for the entire persona's notes
  var allNotesText = notes.map(function(note) {
    var prefix = note.chapter ? '[Ch. ' + note.chapter + (note.label ? ' \u00b7 ' + note.label : '') + '] ' : '';
    return prefix + note.text;
  }).join('\n\n');

  var html = '<div class="notes-copy-all-wrap">'
    + '<button class="copy-btn copy-all-btn" data-copy="' + escHtml(allNotesText) + '">Copy all notes</button>'
    + '</div>';

  html += groupOrder.map(function(key) {
    ...
    // "Copy chapter" text: all notes from this chapter
    var chText = groupNotes.map(function(note) {
      return (note.label ? '[' + note.label + '] ' : '') + note.text;
    }).join('\n\n');
    ...
    var items = groupNotes.map(function(note) {
      ...
      return '<li class="note-item">'
        ...
        + '<button class="copy-btn" data-copy="' + escHtml(note.text) + '">Copy</button>'
        + '</li>';
    }).join('');

    return '<details class="notes-chapter-group"' + openAttr + '>'
      + '<summary class="notes-group-summary">'
      + escHtml(chLabel) + ' <span class="notes-group-count">(' + groupNotes.length + ')</span>'
      + '<button class="copy-btn copy-ch-btn" data-copy="' + escHtml(chText) + '">Copy chapter</button>'
      + '</summary>'
